...
void InitOpenGL() {
#ifdef _WIN32
  HWND hw = CreateWindowExA(0, "EDIT", "", ES_READONLY, 0, 0, 1, 1, NULL, NULL,
...
bool Nama::CheckGLContext() {
  ...
  printf("gl ver test (%s:%d):version:%s  %08x %08x %08x %08x\n", __FILE__,
         __LINE__, OpenGLVersion, add0, add1, add2, add3);
...
bool Nama::Init() {
  if (false == mHasSetup && true == mNamaAppState.EnableNama) {
    ...
    if (false == FuTool::LoadBundle(g_tongue, tongue_model_data)) {
      ...
      fuLoadTongueModel(reinterpret_cast<float*>(&tongue_model_data[0]),
                        tongue_model_data.size());
    }

    vector<char> ai_model_data;
    if (false == FuTool::LoadBundle(g_ai_faceprocessor, ai_model_data)) {
      cout << "Error:fail load faceprocessor model" << g_ai_faceprocessor
           << endl;
    } else {
      fuSetFaceModelConfig(FUAIFACE_ALL_DEFAULT);
      fuSetFaceAlgorithmConfig(FUAIFACE_ENABLE_ALL);
      fuLoadAIModelFromPackage(reinterpret_cast<float*>(&ai_model_data[0]),
                               ai_model_data.size(),
                               FUAITYPE::FUAITYPE_FACEPROCESSOR);
    }

    vector<char> ai_human_model_data;
    if (false == FuTool::LoadBundle(g_ai_humanprocessor, ai_human_model_data)) {
      cout << "Error: fail load humanprocessor model" << g_ai_humanprocessor
           << endl;
    } else {
      // fuSetHumanSegMode(FUAIHUMAN_SEG_GPU_COMMON);
      // fuPreprocessAIModelFromPackage(reinterpret_cast<float*>(&ai_human_model_data[0]),
      // ai_human_model_data.size(), FUAITYPE::FUAITYPE_HUMAN_PROCESSOR);
      fuSetHumanModelConfig(FUAIHUMAN_SEG_GPU_COMM);
      fuSetHumanAlgorithmConfig(FUAIHUMAN_ENABLE_ALL);
      fuLoadAIModelFromPackage(
          reinterpret_cast<float*>(&ai_human_model_data[0]),
          ai_human_model_data.size(), FUAITYPE::FUAITYPE_HUMAN_PROCESSOR);
    }
    vector<char> ai_gesture_model_data;
    if (false ==
        FuTool::LoadBundle(g_ai_hand_processor, ai_gesture_model_data)) {
      cout << "Error: fail load gesture model" << g_ai_hand_processor << endl;
    } else {
      fuLoadAIModelFromPackage(
          reinterpret_cast<float*>(&ai_gesture_model_data[0]),
          ai_gesture_model_data.size(), FUAITYPE::FUAITYPE_HANDGESTURE);
    }
...

    {
      ...
      if (false == FuTool::LoadBundle(g_bodySlim, propData)) {
        ...
        mBodyShapeHandle =
            fuCreateItemFromPackage(&propData[0], propData.size());
      }
    }
...
void Nama::ReloadItems() {
  ...
  m_CMakeupTypeMap.clear();
  {
    vector<char> propData;
    if (false ==
        FuTool::LoadBundle(g_assetDir + g_faceBeautification, propData)) {
      cout << "load face beautification data failed." << endl;
    } else {
      cout << "load face beautification data." << endl;
      mBeautyHandles = fuCreateItemFromPackage(&propData[0], propData.size());
  ...
  m_Controller->InitController(g_assetDir + g_control,
                               g_assetDir + g_control_cfg);
...
#if _WIN32
      cv::Mat mat = cv::imread(UIBridge::mCurRenderGSSAItemName,
                               cv::IMREAD_REDUCED_COLOR_4);
#elif __APPLE__
      cv::Mat mat = cv::imread(FuToolMac::GetDocumentPath() + "/" +
                               GSSAFEAREA_USERFILE_PIC);
#endif
...
#elif __APPLE__
      imagePath = UIBridge::GetFileFullPathFromResourceBundle(
          (gGSSAPic + "/" + UIBridge::mCurRenderGSSAItemName + ".jpg").c_str());
#endif
...
void Nama::SwitchBeauty(bool bValue) { mIsBeautyOn = bValue; }

void Nama::SetCurrentShape(int index) {
  if (false == mNamaAppState.EnableNama || false == mIsBeautyOn ||
      mBeautyHandles == 0)
    return;
  if (0 <= index <= 4) {
  ...
  for (auto param : paramArr) {
    switch (param.type) {
      case MakeupParamType::MArray:
        fuItemSetParamdv(mMakeUpHandle, const_cast<char*>(param.name.c_str()),
                         (double*)param.colorArr.data(), param.colorArr.size());
        break;
      case MakeupParamType::MFloat:
        fuItemSetParamd(mMakeUpHandle, const_cast<char*>(param.name.c_str()),
                        param.brow_warp);
        break;
      ...
      case MakeupParamType::MString: {
        Texture::SharedPtr pTexture =
            Texture::createTextureFromFile(bundleName + "/" + param.tex, false);
        if (pTexture) {
          fuCreateTexForItem(
              mMakeUpHandle, const_cast<char*>(param.name.c_str()),
              pTexture->getData(), pTexture->m_width, pTexture->m_height);
        } else {
...
void Nama::UpdateFilter(int index) {
  ...
  fuItemSetParamd(mBeautyHandles, "filter_level",
                  UIBridge::mFilterLevel[index] / 100.0f);
}
...
void Nama::UpdateBeauty() {
  ...
  for (int i = 2; i < MAX_BEAUTYFACEPARAMTER; i++) {
    if (i == 2) {
      fuItemSetParamd(mBeautyHandles,
                      const_cast<char*>(g_faceBeautyParamName[i].c_str()),
                      UIBridge::mFaceBeautyLevel[i] * 6.0 / 100.f);
    } else if (i == 4) {
      fuItemSetParamd(mBeautyHandles,
                      const_cast<char*>(g_faceBeautyParamName[i].c_str()),
                      UIBridge::mFaceBeautyLevel[i]);
    } else {
      fuItemSetParamd(mBeautyHandles,
                      const_cast<char*>(g_faceBeautyParamName[i].c_str()),
                      UIBridge::mFaceBeautyLevel[i] / 100.f);
    }
  }
  ...
  for (int i = 0; i < MAX_FACESHAPEPARAMTER; i++) {
    ...
    fuItemSetParamd(mBeautyHandles,
                    const_cast<char*>(g_faceShapeParamName[i].c_str()),
                    UIBridge::mFaceShapeLevel[i] / 100.0f);
    /*if (i == 12)
    {
            wchar_t szBuf[2048] = { 0 };
            wsprintf(szBuf,L"liufei set data %d",
    (int)UIBridge::mFaceShapeLevel[i]); OutputDebugString(szBuf);
  ...
  fuItemSetParamd(mBeautyHandles, "blur_type",
                  blurType[UIBridge::mFaceBeautyLevel[1]]);
  fuItemSetParamd(mBeautyHandles, "body_blur_level",
                  UIBridge::mBodyBlurLevel * 6.0 / 100.f);
  fuItemSetParamd(mBeautyHandles, "facial_plump",
                  UIBridge::mFacialPlumpLevel / 100.f);
  fuItemSetParamd(mBeautyHandles, "intensity_eye_pupil",
                  (UIBridge::mEyePupilLevel + 50.f) / 100.f);
  fuItemSetParamd(mBeautyHandles, "face_shape_level", 1);
  if (!(UIBridge::showLightMakeupTip && !UIBridge::showGreenScreen)) {
    fuItemSetParamd(mBeautyHandles, "filter_level",
                    UIBridge::mFilterLevel[UIBridge::m_curFilterIdx] / 100.0f);
  }
}
...
void Nama::LoadRencentColorFromFile() {
  ...
  string json_content((istreambuf_iterator<char>(in)),
...
void Nama::UpdateGreenScreen() {
  ...
  for (int i = 0; i < MAX_GREEN_SCREEN_PARAM; i++) {
    fuItemSetParamd(mGSHandle, const_cast<char*>(g_GSParamName[i].c_str()),
                    UIBridge::mGSParam[i] / 100.f);
  }
#if 0
...
void Nama::UpdateGSBg(cv::Mat& dataRGBA) {
  ...
  fuCreateTexForItem(mGSHandle, "tex_bg", dataRGBA.data, dataRGBA.cols,
                     dataRGBA.rows);
...
void Nama::UpdateGSSA(cv::Mat& dataRGBA) {
  ...
  fuCreateTexForItem(mGSHandle, "tex_template", dataRGBA.data, dataRGBA.cols,
                     dataRGBA.rows);
}

void Nama::NonuseGSSA() { fuItemSetParamd(mGSHandle, "is_use_template", 0); }

// 更新背景分割的背景数据
void Nama::UpdateSegBg(cv::Mat& dataRGBA) {
  if (UIBridge::bundleCategory != BundleCategory::BackgroundSegmentation ||
      UIBridge::m_curRenderItem == -1) {
    return;
  }
  ...
  fuCreateTexForItem(UIBridge::m_curRenderItem, "tex_bg_seg", dataRGBA.data,
                     dataRGBA.cols, dataRGBA.rows);
}
...
void Nama::UpdateBodyShape() {
  ...
  for (int i = 0; i < MAX_BODY_SHAPE_PARAM; i++) {
    fuItemSetParamd(mBodyShapeHandle,
                    const_cast<char*>(g_bodyShapeParamName[i].c_str()),
                    UIBridge::mBodyShapeLevel[i] / 100.0f);
  }
  fuItemSetParamd(mBodyShapeHandle, "BreastStrength",
                  UIBridge::mBreastStrengthLevel / 100.0f);
}
/// 暂停当前MP3
...
void Nama::SetCMDoubles(const std::string& key, double* values,
                        uint32_t count) {
  if (mMakeUpHandle > 0) {
...
void Nama::DestroyAll() {
  ...
  if (UIBridge::bundleCategory == BundleCategory::Makeup &&
      UIBridge::m_curBindedItem != -1 && mMakeUpHandle != -1) {
...
int Nama::GetLastNamaError() { return fuGetSystemError(); }
...
bool Nama::SelectBundle(string bundleName, int maxFace) {
  ...
  if (0 == mBundlesMap[bundleName]) {
...
#elif __APPLE__
        string mp3Path = FuToolMac::GetFileFullPathFromResourceBundle(
            ("items/MusicFilter/" + itemName + ".mp3").c_str());
        mp3->Load(mp3Path);
#endif
        mp3Map[bundleID] = mp3;
      }
    ...
    if (UIBridge::bundleCategory == BundleCategory::StyleRecommendation) {
      StyleRecommendationParam tempSRP =
          UIBridge::mStyleParamList.at(UIBridge::mStyleRecommendationIndex);
      fuItemSetParamd(bundleID, "machine_level", 1.0);
      fuItemSetParamd(bundleID, "filter_level",
                      float(tempSRP.mFilterLevel) / 100);
      fuItemSetParamd(bundleID, "makeup_intensity",
                      float(tempSRP.mMakeUpIntensity) / 100);
    }
  } else {
  ...
  if (UIBridge::m_curRenderItem == bundleID &&
    ...
    // 如果不是美妆，首先解除美妆效果
    if (UIBridge::bundleCategory != BundleCategory::Makeup &&
        mMakeUpHandle != -1 && UIBridge::m_curRenderItem != -1) {
      fuUnbindItems(mMakeUpHandle, &UIBridge::m_curRenderItem, 1);
      UIBridge::m_curBindedItem = -1;
    }
  ...
  if (UIBridge::bundleCategory == Animoji) {
    ...
    vector<string> humanSegmentations = {"boyfriend1", "boyfriend2",
                                         "boyfriend3"};
...
void Nama::RenderDefNama(cv::Mat& picInput, int rotType) {
  if (UIBridge::mNeedPlayMP3) {
    if (mp3Map.find(UIBridge::m_curRenderItem) != mp3Map.end()) {
      fuItemSetParamd(
          UIBridge::m_curRenderItem, "music_time",
          mp3Map[UIBridge::m_curRenderItem]->GetCurrentPosition() / 1e4);
...

  {
    ...
    fuRender(FU_FORMAT_RGBA_BUFFER, (int*)(picInput.data),
             FU_FORMAT_RGBA_BUFFER, (int*)(picInput.data), picInput.cols,
             picInput.rows, mFrameID, renderList.data(),
             (int32_t)renderList.size(), NAMA_RENDER_FEATURE_FULL, NULL);
    ...
    } else if (UIBridge::bundleCategory ==
                   BundleCategory::BackgroundSegmentation ||
...
FURect Nama::getGSPreviewRect() { return gsPreviewRect; }
void Nama::RenderGS(cv::Mat& picInput, int rotType) {
  if (mGSHandle != -1) {
    ...
    fuSetInputCameraBufferMatrix((TRANSFORM_MATRIX)rotType);
    fuSetInputCameraTextureMatrix((TRANSFORM_MATRIX)rotType);
    fuRender(FU_FORMAT_RGBA_BUFFER, (void*)(picInput.data),
             FU_FORMAT_RGBA_BUFFER, (void*)(picInput.data), picInput.cols,
             picInput.rows, mFrameID, vecRender.data(),
             (int32_t)vecRender.size(), NAMA_RENDER_FEATURE_FULL, NULL);
  }
...
void Nama::RenderItems(cv::Mat& picMat) {
  if (UIBridge::showGreenScreen) {
    TRANSFORM_MATRIX type =
        (CCameraManage::getInstance()->mCapture->getCaptureType() ==
         CAPTURE_CAMERA)
            ? TRANSFORM_MATRIX::CCROT0_FLIPHORIZONTAL
            : TRANSFORM_MATRIX::CCROT0;
...
void Nama::DrawLandmarks(uchar* frame) {
  ...
  ret = fuGetFaceInfo(0, "landmarks", landmarks,
                      sizeof(landmarks) / sizeof(landmarks[0]));
  for (int i(0); i != 75; ++i) {
    DrawPoint(frame, static_cast<int>(landmarks[2 * i]),
              static_cast<int>(landmarks[2 * i + 1]));
  }
}

void Nama::DrawPoint(uchar* frame, int x, int y, unsigned char r,
...
BodyTrackType Nama::GetBodyTrackType() { return m_bodyTrackType; }
...
void NamaExampleNameSpace::Nama::setLightMakeupTex(string path, string value) {
  ...
  fuCreateTexForItem(UIBridge::m_curRenderItem, value.data(), image.data,
...
void NamaExampleNameSpace::Nama::setLightMakeupParam(LightMakeupParam param) {
  ...
  param.blusherPath =
      UIBridge::GetFileFullPathFromResourceBundle(param.blusherPath.c_str());
  param.eyeshadowPath =
      UIBridge::GetFileFullPathFromResourceBundle(param.eyeshadowPath.c_str());
  param.eyebrowPath =
      UIBridge::GetFileFullPathFromResourceBundle(param.eyebrowPath.c_str());
  param.lipColorPath =
      UIBridge::GetFileFullPathFromResourceBundle(param.lipColorPath.c_str());
#endif
  setLightMakeupTex(param.blusherPath, "tex_blusher");
  fuItemSetParamd(UIBridge::m_curRenderItem, "makeup_intensity_blusher",
                  param.blusher);
  setLightMakeupTex(param.eyeshadowPath, "tex_eye");
  fuItemSetParamd(UIBridge::m_curRenderItem, "makeup_intensity_eye",
                  param.eyeshadow);
  setLightMakeupTex(param.eyebrowPath, "tex_brow");
  fuItemSetParamd(UIBridge::m_curRenderItem, "makeup_intensity_eyeBrow",
                  param.eyebrow);
  ...
  if (!dom.Parse(json_content.c_str()).HasParseError()) {
    if (dom.HasMember("rgba") && dom["rgba"].IsArray()) {
      ...
      fuItemSetParamdv(UIBridge::m_curRenderItem, "makeup_lip_color",
                       color.data(), color.size());
    }
  }

  fuItemSetParamd(UIBridge::m_curRenderItem, "makeup_intensity",
                  param.intensity);
...
void NamaExampleNameSpace::Nama::setHumanSegScene(int type) {
  ...
  if (FuTool::LoadBundle(g_ai_humanprocessor, ai_human_model_data)) {
    ...
    fuSetHumanAlgorithmConfig(FUAIHUMAN_ENABLE_ALL);
    fuLoadAIModelFromPackage(reinterpret_cast<float*>(&ai_human_model_data[0]),
                             ai_human_model_data.size(),
                             FUAITYPE::FUAITYPE_HUMAN_PROCESSOR);
  }
}

void NamaExampleNameSpace::Nama::setMaxFaces(int face) { fuSetMaxFaces(face); }
